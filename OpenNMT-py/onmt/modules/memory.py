...
class Memory(nn.Module):
  ...
  def __init__(self, input_size, mem_size=512, cell_size=32, read_heads=4, independent_linears=True):
    ...
    self.I = (1 - T.eye(m).unsqueeze(0)).cuda()  # (1 * n * n)
  ...
  def reset(self, batch_size=1, hidden=None, erase=True):
    ...
    if hidden is None:
      return {
          'memory': T.zeros(b, m, w).fill_(0).cuda(),
          'link_matrix': T.zeros(b, 1, m, m).cuda(),
          'precedence': T.zeros(b, 1, m).cuda(),
          'read_weights': T.zeros(b, r, m).fill_(0).cuda(),
          'write_weights': T.zeros(b, 1, m).fill_(0).cuda(),
          'usage_vector': T.zeros(b, m).cuda()
      }
    else:
  ...
  def allocate(self, usage, write_gate):
    ...
    # free list
    sorted_usage, phi = T.topk(usage, self.mem_size, dim=1, largest=False)
    ...
    _, phi_rev = T.topk(phi, k=self.mem_size, dim=1, largest=False)
  ...
  def forward(self, xi, hidden):
    ...
    if self.independent_linears:
      # r read keys (b * r * w)
      read_keys = T.tanh(self.read_keys_transform(xi).view(b, r, w))
      # r read strengths (b * r)
      read_strengths = F.softplus(self.read_strengths_transform(xi).view(b, r))
      # write key (b * 1 * w)
      write_key = T.tanh(self.write_key_transform(xi).view(b, 1, w))
      # write strength (b * 1)
      write_strength = F.softplus(self.write_strength_transform(xi).view(b, 1))
      # erase vector (b * 1 * w)
      erase_vector = T.sigmoid(self.erase_vector_transform(xi).view(b, 1, w))
      # write vector (b * 1 * w)
      write_vector = T.tanh(self.write_vector_transform(xi).view(b, 1, w))
      # r free gates (b * r)
      free_gates = T.sigmoid(self.free_gates_transform(xi).view(b, r))
      # allocation gate (b * 1)
      allocation_gate = T.sigmoid(self.allocation_gate_transform(xi).view(b, 1))
      # write gate (b * 1)
      write_gate = T.sigmoid(self.write_gate_transform(xi).view(b, 1))
      # read modes (b * r * 3)
      read_modes = sigma(self.read_modes_transform(xi).view(b, r, 3), 1)
    else:
      xi = self.interface_weights(xi)
      # r read keys (b * w * r)
      read_keys = T.tanh(xi[:, :r * w].contiguous().view(b, r, w))
      # r read strengths (b * r)
      read_strengths = F.softplus(xi[:, r * w:r * w + r].contiguous().view(b, r))
      # write key (b * w * 1)
      write_key = T.tanh(xi[:, r * w + r:r * w + r + w].contiguous().view(b, 1, w))
      # write strength (b * 1)
      write_strength = F.softplus(xi[:, r * w + r + w].contiguous().view(b, 1))
      # erase vector (b * w)
      erase_vector = T.sigmoid(xi[:, r * w + r + w + 1: r * w + r + 2 * w + 1].contiguous().view(b, 1, w))
      # write vector (b * w)
      write_vector = T.tanh(xi[:, r * w + r + 2 * w + 1: r * w + r + 3 * w + 1].contiguous().view(b, 1, w))
      # r free gates (b * r)
      free_gates = T.sigmoid(xi[:, r * w + r + 3 * w + 1: r * w + 2 * r + 3 * w + 1].contiguous().view(b, r))
      # allocation gate (b * 1)
      allocation_gate = T.sigmoid(xi[:, r * w + 2 * r + 3 * w + 1].contiguous().unsqueeze(1).view(b, 1))
      # write gate (b * 1)
      write_gate = T.sigmoid(xi[:, r * w + 2 * r + 3 * w + 2].contiguous()).unsqueeze(1).view(b, 1)
      # read modes (b * 3*r)
...
def theta(a, b, dimA=2, dimB=2, normBy=2):
