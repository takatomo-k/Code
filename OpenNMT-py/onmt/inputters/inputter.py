...
def _build_field_vocab(field, counter, **kwargs):
    specials = list(OrderedDict.fromkeys(
        tok for tok in [UNK_WORD, PAD_WORD, BOS_WORD, EOS_WORD]
        if tok is not None))
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
    ...
    Args:
        ...
        src_words_min_frequency(int): the minimum frequency needed to include a source word in the vocabulary.
        tgt_vocab_path(string): Path to tgt vocabulary file.
        tgt_vocab_size(int): size of the target vocabulary.
        tgt_words_min_frequency(int): the minimum frequency needed to include a target word in the vocabulary.
    ...
    if src_vocab is not None:
        for i in src_vocab:
            counter['src'].update([i]*(src_words_min_frequency+1))
    if tgt_vocab is not None:
        for i in tgt_vocab:
            counter['tgt'].update([i]*(tgt_words_min_frequency+1))
